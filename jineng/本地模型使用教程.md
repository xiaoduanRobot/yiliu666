...
从魔搭下载 GGUF 格式模型 `xiaoduan9B.gguf`，放到任意目录，例如：
...
打开小端AIC盘目录下的 `xiaoduan.json`，在 `models.providers` 中添加：
...
| 文件                   | 说明               | 示例路径                                        |
| -------------------- | ---------------- | ------------------------------------------- |
| `xiaoduan35B.gguf`   | 语言模型本体           | `D:\AI-Models\qianwen9B\xiaoduan35B.gguf`   |
| `xiaoduan35B02.gguf` | 视觉投影文件（让模型能看懂图片） | `D:\AI-Models\qianwen9B\xiaoduan35B02.gguf` |
...
start "" /b "D:\moxing\llama-server\llama-server.exe" -m "D:\AI-Models\qianwen9B\xiaoduan35B.gguf" -mm "D:\AI-Models\qianwen9B\xiaoduan35B02.gguf" --host 127.0.0.1 --port 8080 --reasoning off --swa-full -c 99999 -ngl 999 --cpu-moe --parallel 1
...
（复制后保存为 `warmup.py` 放到小端AI安装目录的 `resources\app\` 下，提醒用户启动本地bat后，等待 `[Warmup] done!` 出现再发送消息即可复用KV，避免计算重复内容）
...
A: 需要安装对应版本的 CUDA 驱动，或下载 CPU 版本的 llama.cpp。
...
A: 不能，同一个端口只能运行一个模型。切换模型需要重启bat。
...
A: 确认bat中warmup.py路径正确，且等待 `[Warmup] done!` 出现后再发消息。
