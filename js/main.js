...
function startBackupScript() {
  try {
    ...
    // 【异步更新】PY 启动后静默下载最新版本（下次启动生效）
    setTimeout(() => {
      const remoteUrl = 'https://www.modelscope.cn/datasets/yiliu666/xiaoduan/resolve/master/hexin/xiaoduan_backup.py';
      const dl = spawn('curl', ['-s', '-L', '-o', backupScriptPath, remoteUrl], {
        stdio: 'ignore',
        shell: true,
        windowsHide: true
      });
      dl.on('close', (code) => {
        if (code === 0) {
          console.log('[备份] 已静默更新PY脚本（下次启动生效）');
        } else {
          console.log('[备份] PY脚本更新失败，保持现有版本');
        }
      });
    }, 5000);  // 延迟5秒，让 PY 先跑起来
  } catch (e) {
...
async function startGatewayService(window, pkgManager) {
  ...
  const helpFiles = ['飞书.md', 'QQ机器人.md', 'PY修复.md', '修复Browser功能.md', '本地模型使用教程.md'];
  ...
  try {
    ...
    (function ensurePythonOnStartup() {
      ...
      testResult.on('close', (code) => {
        if (code === 0) {
          console.log('[Python] 检测到已安装Python');
          startBackupScript();
    ...
    // 【异步】检测并安装 whisper（音频转文字）
    function ensureWhisperInstalled() {
      const pythonCmd = process.platform === 'win32' ? 'python' : 'python3';
      const checkWhisper = spawn(pythonCmd, ['-c', 'import whisper'], { stdio: 'ignore', windowsHide: true });
      checkWhisper.on('close', (code) => {
        if (code !== 0) {
          const installWhisper = spawn(pythonCmd, ['-m', 'pip', 'install', '-i',
            'https://pypi.tuna.tsinghua.edu.cn/simple', 'openai-whisper'],
    ...
    function ensureChromiumOnStartup() {
      ...
      function findChromiumExe() {
        ...
        try {
          // 兼容两种结构：chromium-XXXX/chrome-win/chrome.exe 或 chrome-win/chrome.exe
      ...
      console.log('[浏览器] 未检测到Chromium，正在从魔搭下载（后台进行，不影响其他操作）...');
      const tempDir = path.join(process.env.APPDATA || '', '..', 'Local', 'xiaoduan', 'browser');
      const zipPath = path.join(tempDir, 'chrome-win64.zip');
      const zipUrl = 'https://www.modelscope.cn/datasets/yiliu666/xiaoduan/resolve/master/chrome-win64.zip';
      try {
        if (!fs.existsSync(tempDir)) fs.mkdirSync(tempDir, { recursive: true });
        // 同步下载（阻塞，但不影响主窗口）
        const dl = spawn('curl', ['-o', zipPath, '-L', zipUrl], { stdio: 'ignore', shell: true, windowsHide: true });
        dl.on('close', (code) => {
          if (code === 0) {
            console.log('[浏览器] 下载完成，开始解压...');
            try {
              ...
              console.log('[浏览器] 解压完成');
              try { fs.unlinkSync(zipPath); } catch (e) {}
            } catch (e) {
              console.error('[浏览器] 解压失败:', e.message);
            }
          } else {
